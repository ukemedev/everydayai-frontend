...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .studio-split { display: flex; height: calc(100vh - 48px); overflow: hidden; }
  .studio-left { width: 400px; min-width: 300px; flex-shrink: 0; border-right: var(--border); display: flex; flex-direction: column; overflow: hidden; background: var(--black-950); }
  .studio-left-header { padding: 20px 20px 0; flex-shrink: 0; }
  .studio-left-body { flex: 1; overflow-y: auto; padding: 0 20px 24px; }
  .studio-right { flex: 1; display: flex; flex-direction: column; overflow: hidden; min-width: 0; background: var(--black); }
  .studio-chat-header { padding: 12px 20px; border-bottom: var(--border); flex-shrink: 0; display: flex; align-items: center; gap: 10px; background: var(--black-950); }
  .studio-chat-log { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 14px; }
  .studio-chat-input { padding: 12px 20px; border-top: var(--border); display: flex; gap: 8px; align-items: center; flex-shrink: 0; background: var(--black-950); }
  ...
  .tools-table td { padding: 9px 8px; border-bottom: var(--border); color: var(--gray-400); vertical-align: middle; }
  ...
  .method-get { background: rgba(0,255,136,0.08); color: var(--green-term); border: 1px solid rgba(0,255,136,0.2); }
          ...
          {tab === 1 && (
            <div style={{ paddingTop: 20 }} className="page-enter">
              ...
              <div style={{ fontSize: 10, color: "var(--gray-600)" }}>
                {knowledge.trim()
                  ? `// ${knowledge.split("\n").filter(l => l.trim()).length} lines indexed`
                  : "// no content yet — chat will use prompt only"}
              </div>
            </div>
          )}
              ...
              {tools.length > 0 && (
                <table className="tools-table">
                  ...
                  <tbody>
                    {tools.map((tool: any) => (
                      <tr key={tool.id}>
                        <td style={{ color: "var(--gray-200)", fontWeight: 500 }}>{tool.name}</td>
