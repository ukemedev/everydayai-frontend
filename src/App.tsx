...
function StudioPage({ toast }: { toast: (m: string) => void }) {
  const [tab, setTab] = useState(0);
  const [msgs, setMsgs] = useState<Message[]>([
    { role: "agent", text: "Hello! I'm SupportBot. How can I help you today?" }
  ]);
  const [input, setInput] = useState("");
  const [typing, setTyping] = useState(false);
  const [files, setFiles] = useState<string[]>(["context.pdf", "faq.md"]);
  ...
  function send() {
    ...
    setTimeout(() => {
      setTyping(false);
      setMsgs(m => [...m, { role: "agent", text: "I've received your message. This is a demo response from the agent." }]);
    }, 1400);
  }

  return (
    <div className="page page-enter studio-wrap">
      <div className="term-line">agent studio</div>
      <div className="section-header" style={{ marginBottom: 0 }}>
        <div className="section-title">SupportBot</div>
        <span className="status-badge status-live"><span className="status-dot" />live</span>
      </div>
            ...
            <div className="chat-log" ref={logRef}>
              {msgs.map((m, i) => (
                <div key={i} className={`msg-row${m.role === "user" ? " user" : ""}`}>
                  <span className={`msg-label ${m.role === "agent" ? "agent-lbl" : "user-lbl"}`}>{m.role === "agent" ? "AGT" : "YOU"}</span>
                  <div className={`msg-bubble ${m.role}`}>{m.text}</div>
                </div>
              ))}
              {typing && (
                <div className="msg-row">
                  <span className="msg-label agent-lbl">AGT</span>
                  <div className="typing"><span className="t-dot" /><span className="t-dot" /><span className="t-dot" /></div>
                </div>
              )}
            </div>
        ...
        <div>
          {files.map(f => (
            <div key={f} className="file-row">
              ...
              <button className="btn btn-danger btn-sm" onClick={() => { setFiles(fs => fs.filter(x => x !== f)); toast("File removed."); }}>Remove</button>
            </div>
          ))}
          <div className="upload-zone" style={{ marginTop: 16 }} onClick={() => toast("File upload coming soon.")}>
            <div className="upload-ascii">{"  ╔══════╗\n  ║  ↑   ║\n  ╚══════╝"}</div>
            <div className="upload-text">Click to upload files</div>
            <div className="upload-sub">PDF, MD, TXT · max 10MB</div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="app">
      ...
      <div className="layout">
        <Sidebar page={page} setPage={setPage} email={user} onLogout={() => setUser(null)} />
        <main className="main">
